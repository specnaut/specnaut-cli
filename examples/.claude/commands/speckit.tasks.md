...
5. **Report**: Output path to generated tasks.md and summary:
   ...
   - **Suggested next command** — always inside a fenced code block for easy
     copy:
...
4. **Frontend Architecture Rules (CRITICAL)**:
   - MUST place all new business components, hooks, and stores in `inertia/features/<domain>/`
   - MUST NOT place new business logic in the generic `inertia/components/` directory
   - MUST use Inertia's `useForm` for all forms with `<FieldError name="...">` component
   - MUST use Zustand exclusively for ephemeral client state or SSE, never duplicating Server State (DB rows)
