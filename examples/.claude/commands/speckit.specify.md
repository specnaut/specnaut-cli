...
Extract the mode flag from the feature description:

- `--manual` → legacy single-shot mode (already documented)
- `--copilot` → cloud handoff mode (NEW — see below)
- (no flag) → default auto mode

If `--copilot` is present:

1. **Extract backlog-id and slug.** The user must have already created
   the backlog task. Ask them which backlog task this spec corresponds
   to if not obvious from the description. Fail loudly if no matching
   `tasks/backlog/NNN-*.md` exists.

2. **Run the bootstrap script** instead of `create-new-feature.sh`:

   ```bash
   python3 .claude/skills/speckit/scripts/create-linked-branch.py \
       --backlog-id NNN --slug <derived-slug>
   ```

   Capture the JSON output: `{"branch": "...", "issue_number": N, "issue_url": "..."}`.

3. **Store these values** for Phase 3 handoff:
   - `BRANCH_NAME` = from script output
   - `ISSUE_NUMBER` = from script output
   - `FEATURE_DIR` = `specs/<issue_number>-<slug>/`

4. **Skip the legacy `create-new-feature.sh` step below** and jump
   straight to loading the spec template.

If `--copilot` is NOT present, follow the existing flow unchanged.
...
2. **Check for existing branches before creating new one**:
   ...
   **IMPORTANT**:
   - Check all three sources (remote branches, local branches, specs
     directories) to find the globally highest numerical prefix (e.g. 095)
   - Do NOT filter by the short-name when determining the highest number; you want the global maximum across all features.
   - If no existing branches/directories found with a number, start with number 1
...
### Phase 3 handoff (only under `--copilot`)

When the auto-chain reaches the post-`analyze` point and `--copilot` was
the initial flag:

- Skip `implement`, `review`, and `merge`.
- Invoke the handoff script:
  ...
  python3 .claude/skills/speckit/scripts/handoff-to-copilot.py \
      ...
      --branch "$BRANCH_NAME" \
      --title "<feature title derived from spec.md>"
  ```
...
- Report to the user:
  ...
  ```
  ✅ Handoff complete.
     PR: <pr_url> (draft, awaiting Copilot)
     Preview: will populate when preview.yml finishes (~5-10 min).
     Branch: <branch> now belongs to Copilot — further local commits will conflict.
  ```

7. Report completion with branch name, spec file path, checklist results, and
   readiness for the next phase. **Always suggest the next command inside a
   fenced code block** so the user can copy it with one click:
