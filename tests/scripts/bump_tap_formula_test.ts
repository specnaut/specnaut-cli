...
Deno.test("renderFormula produces a Ruby Formula class with the right shape", () => {
  ...
  assertStringIncludes(out, "class Specnaut < Formula");
  assertStringIncludes(out, 'version "1.2.3"');
  assertStringIncludes(out, "v1.2.3/specnaut-macos-arm64");
  assertStringIncludes(out, "v1.2.3/specnaut-macos-x64");
  assertStringIncludes(out, "v1.2.3/specnaut-linux-arm64");
  assertStringIncludes(out, "v1.2.3/specnaut-linux-x64");
...
Deno.test("renderFormula keeps Ruby string interpolation in the test block", () => {
  ...
  // time.
  assertStringIncludes(out, "/^specnaut #{version}/");
});
