...
- Returns the canonical path of the **first** ancestor `A` such that:
  - `A/.specnaut/` exists (directory), **and**
...
- Returns `true` iff `targetDir/.specnaut/standalone.yml` exists. Contents ignored.
...
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------ |
| **C1** member target ⇒ init writes 0 agentic files | `tests/integration/init_parent_managed_test.ts::parent-managed suppresses agentic`                                   | 0 files under `target/.claude/skills` & `target/.claude/agents`; `.specnaut/` present; notice printed | SC-001, FR-005/006 |
...
| **C3** upgrade on parent-managed ⇒ no resurrection | `tests/integration/upgrade_parent_managed_test.ts::no agentic resurrection`                                          | upgrade updates `.specnaut/`; 0 agentic files; no `.claude/` recreated                                | SC-003, FR-007     |
