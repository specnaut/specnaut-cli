...
```json
{"ts":"2026-06-13T11:00:00Z","event":"stop","session":"<id|unknown>","agent":"<name|unknown>"}
```
...
- The four base fields are always present. The five contract fields (`state`, `done_criteria_met`,
  `handoff_target`, `review_verdict`, `qa_verdict`) are each OPTIONAL and present only when parsed from
  the agent output — never emitted empty.
