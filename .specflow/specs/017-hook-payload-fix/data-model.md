...
Base (unchanged): `ts`, `event`, `session`. Corrected: `agent` ← `agent_type` (was always
`unknown`). New optional: `agent_id` (from `agent_id`), `effort` (from `effort.level`).
...
- output:
  `.last_assistant_message // .output // .result // .response // .tool_response // .message // (raw)`
- agent_id: `.agent_id` (optional)
- effort: `.effort.level` (optional)
- block fields: case-insensitive grep of `STATE:` / `DONE_CRITERIA_MET:` / `HANDOFF_TARGET:`
  (WORKFLOW segment), `REVIEW_VERDICT:` (REVIEW segment), `QA_VERDICT:` (QA segment)
