#!/usr/bin/env bash
# Sync Specnaut → mkrlabs/specnaut-marketplace catalog.
#
...
#
#   - Claude Code: `/plugin marketplace add mkrlabs/specnaut-marketplace`
#                  + `/plugin install specnaut@specnaut-marketplace`
#   - Copilot CLI: `copilot plugin marketplace add mkrlabs/specnaut-marketplace`
...
#              requests:write on mkrlabs/specnaut-marketplace.
...
# Configuration.
MARKETPLACE="mkrlabs/specnaut-marketplace"
...
BODY=$(cat <<EOF
Automated marketplace catalog bump from \`mkrlabs/specnaut\` v$VERSION.
...
[mkrlabs/specnaut](https://github.com/mkrlabs/specnaut).
EOF
)
