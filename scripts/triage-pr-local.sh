...
#
# Looks for open PRs on mkrlabs/specflow that don't have a 🤖 marker review
...
# ~/Library/Logs/specflow/triage-pr.log.
#
...
echo "=== specflow-triage-pr START $(date -Iseconds) (cwd=$REPO_ROOT) ==="
...
# calm.
NEEDS_TRIAGE=$(gh pr list --repo mkrlabs/specflow --state open --json number,comments \
  --jq '[.[] | select(([.comments[] | select(.body | startswith("🤖 specflow-triage-pr review:"))] | length) == 0) | .number] | join(",")')

if [[ -z "$NEEDS_TRIAGE" ]]; then
  echo "no open PRs need triage — exiting"
  echo "=== specflow-triage-pr END $(date -Iseconds) ==="
  exit 0
fi

echo "PRs needing triage: $NEEDS_TRIAGE"

PROMPT="You are running as a scheduled PR triage routine for the Specflow project. Your job: review any open PRs on mkrlabs/specflow that don't yet have a triage review, and post a structured review comment.

Open PR numbers needing review (comma-separated): $NEEDS_TRIAGE

For each PR number:

1. \`gh pr view <num> --repo mkrlabs/specflow --json title,body,headRefName,baseRefName,additions,deletions,changedFiles,files\` to get metadata.
2. \`gh pr diff <num> --repo mkrlabs/specflow\` to read the full diff.
...
Then post ONE review comment per PR via \`gh pr comment <num> --repo mkrlabs/specflow --body \"<review>\"\` with this exact structure (always include the marker prefix on the FIRST line):

\`\`\`
🤖 specflow-triage-pr review:
...
- One comment per PR. The marker prefix '🤖 specflow-triage-pr review:' MUST be on the first line so future runs skip already-reviewed PRs.
