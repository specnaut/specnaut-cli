...
const BACKLOG_CONFIG_STUB = `# backlog-config.yml — generated by \`specflow init --backlog github\`
...
async function writeBacklogConfigStub(targetDir: string): Promise<void> {
  ...
  await Deno.writeTextFile(path, BACKLOG_CONFIG_STUB);
  console.log(
    dim(
...
export async function runInit(intent: InitIntent): Promise<number> {
  ...
  if (backlogBackend === "github") {
    await writeBacklogConfigStub(targetDir);
  }
  console.log("\nNext steps:");
  console.log(
