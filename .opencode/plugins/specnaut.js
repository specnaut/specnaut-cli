/**
 * Specnaut plugin for OpenCode.ai
 *
 * Injects Specnaut bootstrap context via system prompt transform.
 * Auto-registers the bundled skills directory via config hook — no
 * symlinks needed.
 *
 * Inspired by obra/superpowers v5.1.0 (MIT) —
 * .opencode/plugins/superpowers.js. Re-implemented for Specnaut's
...
// bootstrap — the adapter must work even if no other Specnaut tooling
// is installed).
...
export const SpecnautPlugin = async ({ client: _client, directory: _directory }) => {
  const homeDir = os.homedir();
  // Specnaut's plugin source lives at `plugin/skills/` relative to the
  // repo root; this adapter at `.opencode/plugins/specnaut.js` walks
  // up two levels to reach the repo root, then into plugin/skills.
  const specnautSkillsDir = path.resolve(__dirname, "../../plugin/skills");
  ...
  const getBootstrapContent = () => {
    ...
    // plugin/skills/using-specnaut/references/opencode-tools.md, just
    // pre-resolved so the agent doesn't need a second Read on session
    // start.
    const toolMapping = `**Tool Mapping for OpenCode:**
When Specnaut skills reference Claude Code tools, substitute OpenCode equivalents:
- \`TodoWrite\` → \`todowrite\`
...
See plugin/skills/using-specnaut/references/opencode-tools.md for the
full mapping table.`;

    _bootstrapCache = `<EXTREMELY_IMPORTANT>
You have Specnaut.

**IMPORTANT: The using-specnaut bootstrap skill content is included below. It is ALREADY LOADED — you are currently following it. Do NOT use the skill tool to load "using-specnaut" again; that would be redundant.**
...
${toolMapping}
  ...
  return {
    ...
    // Specnaut skills without requiring manual symlinks or config file
    // edits. Config.get() returns a cached singleton — modifications
    // here are visible when skills are lazily discovered later.
    config: async (config) => {
      config.skills = config.skills || {};
      config.skills.paths = config.skills.paths || [];
      if (!config.skills.paths.includes(specnautSkillsDir)) {
        config.skills.paths.push(specnautSkillsDir);
      }
    },
