...
1. **Output source = `last_assistant_message`** (the agent's final message, where end-of-turn blocks
   ...
   version drift.
2. **Agent name = `agent_type`** + fallbacks. Also capture **`agent_id`** (optional) —
   `/status-audit` groups by agent, and two concurrent `security-auditor`s share `agent_type` but
   differ by `agent_id`. Capture **`effort.level`** (optional, cheap context).
3. **Field greps = canonical UPPERCASE names, case-insensitive** (`grep -iE 'STATE:…'`,
   `'REVIEW_VERDICT:…'`, `'QA_VERDICT:…'`, `'DONE_CRITERIA_MET:…'`, `'HANDOFF_TARGET:…'`).
   `REVIEW_VERDICT` and `QA_VERDICT` are already distinct names, so cross-contamination is fixed by
   the names alone; per-block segmentation stays as defense-in-depth.
4. **Test fixture = real shape.** The #381 test fed a synthetic `.output`-keyed payload with
   lowercase fields, which is exactly why the bug shipped green. Replace/augment with a
   `SubagentStop`-shaped fixture (`agent_type` + `last_assistant_message` + UPPERCASE block fields),
   plus a legacy-key case and a lowercase-field case for the fallbacks.

## Why the unit tests missed it

The #381 hermetic test asserted enrichment against an ASSUMED payload shape (the keys the hook
probed), so the test and the hook agreed with each other but neither matched reality. The fix's test
must be anchored to the captured real shape (FR-007).
